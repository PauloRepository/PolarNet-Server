...
const authenticateToken = async (req, res, next) => {
  ...
  try {
    ...
    const userQuery = 'SELECT user_id as userId, username, role, company_id as companyId FROM users WHERE user_id = $1 AND is_active = true';
...
const isProvider = requireRole('PROVIDER');

// Middleware específico para administradores
const isAdmin = requireRole('ADMIN');

// Middleware que permite ambos roles de empresa
const isClientOrProvider = requireRole(['CLIENT', 'PROVIDER']);
...
const requireOwnCompany = async (req, res, next) => {
  ...
  if (!['CLIENT', 'PROVIDER'].includes(req.user.role)) {
